...
void draw()
{
  ...
  for (int index=0; index < width; index++)
  {
    ...
    float timeVal = millis()*0.0001;

    float sinVal1 = sin( map(index, 0, width, 0, periods*2*sin(timeVal*4)*TWO_PI) );
    float sinVal2 = sin( map(index, 0, width, 0, periods*sin(timeVal)*TWO_PI));
