// Tron Blur Revisited Frame differencing Version (for Processing 2.0+)
//
...
void setup() {
  ...
  diffFrameImg = createImage(width, height, ARGB);
...
void next() {
  ...
  grid.wipe();
  grid.setRez((int)(random(2.0, 6.0)));
  walkers.clear();
...
void handleCamera()
{
  ...
  context.update();
    ...
    PImage camImage = context.irImage();
    camImage.loadPixels();
    ...
    { 
      ...
      color currColor = camImage.pixels[i];
      ...
      if (brightness(currColor) > brightness(prevColor))
      {
        ...
        if (usingCamera)
        {
          ...
          int ix = (i % camImage.width) / (grid.rez/2);
          int iy = i/camImage.width;
          iy /= (grid.rez/2);
